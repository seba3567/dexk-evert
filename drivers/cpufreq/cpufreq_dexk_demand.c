/*
 *  drivers/cpufreq/cpufreq_dexk_demand.c
 *
 ...
 *                 2020 Minor Edits by Seba_3567 <sebastian26481@gmail.com>
 *                          teory port to dexk kernel specific changes for increase 
 *                          battery life and performance optimize more...}
 *
 *
...
static u64 dexk_demand_freq_boosted_time;
...
static int dexk_demand_sampling_rate_boosted;
static u64 dexk_demand_sampling_rate_boosted_time;
unsigned int dexk_demand_current_sampling_rate;

static void do_dbs_timer(struct work_struct *work);
static int cpufreq_governor_dbs(struct cpufreq_policy *policy,
				unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_dexk_demand
static
#endif
struct cpufreq_governor cpufreq_gov_dexk_demand = {
       .name                   = "dexk_ondemand",
...
static void dexk_demand_powersave_bias_init_cpu(int cpu)
{
...
static void dexk_demand_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		dexk_demand_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_dexk_demand_power Governor Tunables */
				...
				const char *buf, size_t count)
{
	...
	dexk_demand_freq_boosted_time = ktime_to_us(ktime_get());

	if (dexk_demand_sampling_rate_boosted) {
		dexk_demand_sampling_rate_boosted = 0;
		dbs_tuners_ins.sampling_rate = dexk_demand_current_sampling_rate;
	}
	return count;
}
				   ...
				   const char *buf, size_t count)
{
	...
	dexk_demand_current_sampling_rate = dbs_tuners_ins.sampling_rate;
				    ...
				    const char *buf, size_t count)
{
	...
	dexk_demand_powersave_bias_init();
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (dbs_tuners_ins.boosted && policy->cpu == 0) {
		if (ktime_to_us(ktime_get()) - dexk_demand_freq_boosted_time >=
	...
	if (dexk_demand_sampling_rate_boosted && policy->cpu == 0) {
		if (ktime_to_us(ktime_get()) - dexk_demand_sampling_rate_boosted_time >=
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		dexk_demand_powersave_bias_init_cpu(cpu);
		/*
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	err = cpufreq_register_governor(&cpufreq_gov_dexk_demand);
	if (err) {
		goto error_reg;
	}
...
{
	cpufreq_unregister_governor(&cpufreq_gov_dexk_demand);
	kfree(&dbs_tuners_ins);
}

MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_dexk_demand' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition and save battery  for deluxe kernel by seba_3567");
MODULE_LICENSE("GPLv2");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_DEXK_DEMAND
